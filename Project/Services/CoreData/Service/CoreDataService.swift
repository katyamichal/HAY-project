...
// MARK: - ICoreDataService Implementation
...
extension CoreDataService: ICoreDataService {
    ...
    func add(productType: EntityType, product: IProductCDO) {
        ...
        switch productType {
        case .favourite:
            handleFavouriteProductAddition(product, in: context)
        case .basket:
            handleBasketProductAddition(product, in: context)
        }
    }
    ...
    func deleteProduct(productType: EntityType, id: Int, completion: @escaping (Result<String, Error>) -> Void) {
        ...
        case .favourite:
            if let product = fetchSingleProduct(type: .favourite, id: id) as? HayProduct {
                if let basketProduct = fetchSingleProduct(type: .basket, id: id) as? Basket {
                    product.isFavourite = false
                    completion(.success("Removed from favorites but retained in basket"))
                } else {
                    deleteProductFromContext(productType: .favourite, id: id, completion: completion)
                }
            } else {
                completion(.failure(NSError(domain: "Product could not be found in favorites", code: 404, userInfo: nil)))
            }
        case .basket:
        ...
        do {
            if let basketProduct = try context.fetch(request).first {
                ...
                if basketProduct.count == 0 {
                    ...
                    completion(.success("Successfully updated basket count"))
                }
            }
        } catch {
...
private extension CoreDataService {
    ...
    // MARK: - Handle Favourite Product Addition

    func handleFavouriteProductAddition(_ product: IProductCDO, in context: NSManagedObjectContext) {
    ...
    // MARK: - Handle Basket Product Addition

    func handleBasketProductAddition(_ product: IProductCDO, in context: NSManagedObjectContext) {
        let basketProduct = Basket(context: context)
        basketProduct.productId = Int16(product.productId)
        basketProduct.count = 1
    }
    
    // MARK: - Handle Favourite Product Deletion

    func handleFavouriteProductDeletion(id: Int, completion: @escaping (Result<String, Error>) -> Void) {
        if let product = fetchSingleProduct(type: .favourite, id: id) as? HayProduct {
    ...
    func fetchProducts(by productType: EntityType) throws -> [IProductCDO] {
        ...
        switch productType {
        case .favourite:
            return try fetchFavouriteProducts(in: context)
        case .basket:
            return try fetchBasketProducts(in: context)
        }
    }

    // MARK: - Fetch Favourite Products

    func fetchFavouriteProducts(in context: NSManagedObjectContext) throws -> [IProductCDO] {
    ...
    // MARK: - Fetch Basket Products

    func fetchBasketProducts(in context: NSManagedObjectContext) throws -> [IProductCDO] {
    ...
    // MARK: - Fetch Single Product by Type
    ...
    // MARK: - Delete Product from Context
    ...
    func deleteProducts<T: NSManagedObject>(from request: NSFetchRequest<T>, in context: NSManagedObjectContext, completion: @escaping (Result<String, Error>) -> Void) {
        do {
            ...
            completion(.success("Successfully deleted product"))
        } catch {
            completion(.failure(error))
